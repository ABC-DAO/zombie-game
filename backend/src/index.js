...
let zombieRoutes, tippingRoutes, userRoutes, farcasterRoutes, debugRoutes, apiDebugRoutes;

try {
  console.log('🔍 INDEX: Importing routes...');
  zombieRoutes = require('./routes/zombie');
  ...
  console.error('❌ INDEX: Failed to import routes:', error);
}
...
if (zombieRoutes) {
  try {
    ...
    console.log('✅ INDEX: Zombie routes registered at /api/zombie');
  } catch (error) {
    console.error('❌ INDEX: Failed to register zombie routes:', error);
  }
}
...
app.get('/api/health', (req, res) => {
  ...
  res.json({
    ...
    service: 'zombiefication-backend',
    version: '1.0.0'
  });
});
...
app.get('/', (req, res) => {
  res.json({
    message: 'ZOMBIEFICATION Backend API - Halloween Zombie Game via Farcaster',
    version: '1.0.0',
    endpoints: [
      '/api/health',
      '/api/zombie',
...
async function startServer() {
  ...
  app.listen(PORT, () => {
    ...
    logger.info(`🧟‍♂️ ZOMBIEFICATION Backend running on port ${PORT}`);
    logger.info(`Environment: ${process.env.NODE_ENV || 'development'}`);
    logger.info('🦷 Halloween Zombie Bite Game with Farcaster Integration Ready');
